...
namespace network{
  ...
  struct userList;  //lista di adiacenza degli amici

  struct group;

  struct usrGroupList; //lista di adiacenza dei gruppi

  struct st_Network;  

  typedef st_Network *Network;
  const Network emptyNetwork = NULL;
  
  typedef user* User;
  const User emptyUser = NULL;

  typedef group* Group;
  const Group emptyGroup = NULL;
